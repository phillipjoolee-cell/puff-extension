"""Stability test harness for portfolio generator.

Runs A/B comparison of diversification weights and computes detailed metrics
on portfolio quality, diversity, and constraint compliance.
"""
...
def compute_slip_ev(slip_legs) -> Optional[float]:
    ...
    Args:
        slip_legs: List of SlipLeg objects from a Slip
    
    Returns:
        Average EV % or None if not available
    """
...
def compute_slip_hit_prob(slip_legs) -> Optional[float]:
    ...
    Returns:
        Average hit probability % or None if not available
    """
...
def compute_pairwise_overlap(slips) -> float:
    ...
    Overlap is defined as: shared_legs / slip_size (assuming equal slip sizes).
    
    Args:
        slips: List of Slip objects
    
    Returns:
        Average overlap ratio (0.0 to 1.0)
    """
    if len(slips) < 2:
        return 0.0
...
def compute_player_exposure(slips, max_exposure: float) -> Tuple[float, bool, List[Tuple[str, int]]]:
    ...
    Args:
        slips: List of Slip objects
        max_exposure: Max allowed exposure ratio (e.g., 0.3)
    
    Returns:
        Tuple of:
        - max_player_exposure_ratio (0.0 to 1.0)
        - has_violations (bool)
        - top_5_players (list of (player_name, count))
    """
    if not slips:
        return 0.0, False, []
...
def compute_portfolio_metrics(
    ...
    Args:
        ...
        max_player_exposure: Max exposure cap used during generation
...
def run_ab_test(legs: List[Leg], dataset_name: str, settings: PortfolioSettings) -> Tuple[PortfolioMetrics, PortfolioMetrics]:
    """Run A/B test with different diversification weights.
    
    Args:
        legs: List of legs
        dataset_name: Name of dataset
        settings: Base portfolio settings
    
    Returns:
        Tuple of (metrics_no_diversification, metrics_with_diversification)
    """
    # Monkey-patch portfolio generation to control diversification weights
...
def write_csv_report(all_metrics: List[PortfolioMetrics], output_path: str) -> None:
    ...
    Args:
        all_metrics: List of PortfolioMetrics
